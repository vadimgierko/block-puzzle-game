...
export function drawBlocksToPickFromSection() {
	...
	blocksToPickFrom.forEach((b, i) => {
		const BLOCK_DIV = document.createElement("table");
		// make block draggable
		BLOCK_DIV.setAttribute("draggable", true);
		BLOCK_DIV.style.display = "inline-block";
		BLOCK_DIV.style.margin = "0.5em";

		BLOCK_DIV.addEventListener("dragstart", (e) => {
			console.log(i, "block is dragging");

			if (e.dataTransfer) {
				e.dataTransfer.effectAllowed = "move";
				e.dataTransfer.setData("text/plain", String(i));
				// Use the block element as the drag preview image.
				e.dataTransfer.setDragImage(BLOCK_DIV, 10, 10);
			}
		...
		for (let r = 0; r < b.h; r++) {
			...
			for (let c = 0; c < b.w; c++) {
				...
				const basicCellStyle = "width: 40px; height: 40px;";
